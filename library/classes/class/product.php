...
class class_product extends Zend_Db_Table_Abstract
{
   //declare table variables
	...
	public function remove($code) {
		return $this->delete('product_code = ?', $code);		
	}
	...
	{		
	   ...
	   $result = $this->_db->fetchRow($select);
		...
		if($result) {
			...
			$result['productprices']	= $this->_productprice->getByProduct($result['product_code']);
		}
	...
	{		
		...
		if($result) {			
			for($i = 0; $i < count($result); $i++) {
				...
				$result[$i]['productprices']	= $this->_productprice->getByProduct($result[$i]['product_code']);
			}
		}
